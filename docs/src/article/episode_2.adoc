...
タスクを自動実行するためのgemを追加します。 https://github.com/guard/guard[Guard^]とそのプラグインの https://github.com/guard/guard-shell[Guard::Shell^] https://github.com/guard/guard-minitest[Guard::Minitest^]をインストールします。それぞれの詳細は以下を参照してください。
