...
テストが失敗しました。それは文字列1しか返さないプログラムなのだから当然ですよね。では1が渡されたら文字列1を返し、2を渡したら文字列2を返すようにプログラムを修正しましょう。 Rubyで文字列に変換するメソッドは `to_s` と使ってこのように書きます。
...
引き続き *TODOリスト* を片付けたいのですが `1から100までの数` を返すプログラムを書かないといけません。3を渡したらFizzのような *リテラル* を返すプログラムではなく 1から100までの *配列* を返すようなプログラムにする必要がありそうですね。*TODOリスト* にするとこんな感じでしょうか。
...
結果を配列にして返したいのですが `each` メソッドでうまくできませんね。ここは `map` メソッドを使いましょう。
...
うまくいきましたね。Rubyではreturn文を省略できるので
...
*パイプラインによるループの置き換え*　の適用により `each` メソッドによる繰り返し処理を `map` メソッドを使ったイミュータブルな処理に変えることができました。
