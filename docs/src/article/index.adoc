...
ここでは *アルゴリズムの置き換え* を適用してより　*メソッドチェーンと述語メソッド* を使ってRubyらしい書き方にリファクタリングしてみました。
...
どうやら *配列オブジェクト* を返すプログラムを書かないといけないようですね。え？ *明白な実装* の実装イメージがわかない。そんな時は *仮実装* からステップを小さくして始めるとしましょう。
...
[quote, '_プロを目指す人のためのRuby入門_ <<pruby>>']
____
...
結果を配列にして返したいのですが *eachメソッド* でうまくできませんね。Rubyには新しい配列を作成する *mapメソッド* 用意されいるのでそちらを使いましょう。
...
*パイプラインによるループの置き換え*　の適用により *eachメソッド* による繰り返し処理を *mapメソッド* を使ったイミュータブルな処理に変えることができました。
...
意味のわかる定数として宣言しました。コードに直接記述された `100` をといった *リテラル* はマジックナンバーと呼ばれ往々にして後で何を意味するものかわからなくなり変更を難しくする原因となります。早めに意味を表す定数にしておきましょう。
...
先程は *p* メソッドを使って画面に *配列* の中身を１件ずつ表示していましたが今回は *配列* 自体を開業して画面に出力するため *puts* メソッドを使います。機能的にはほどんど変わらないのですが以下の様に使い分けるそうです。

[quote, '_プロを目指す人のためのRuby入門_ <<prudy>>']
____
...
./lib/fizz_buzz.rb
----
include::./code/lib/fizz_buzz.rb[]
----

./test/fizz_buzz_test.rb
----
include::./code/test/fizz_buzz_test.rb[]
----
...
----
include::./code/test/fizz_buzz_tfd_test.rb[]
----
...
- テスト駆動開発[[tdd]] Kent Beck (著), 和田 卓人 (翻訳):
  オーム社; 新訳版 (2017/10/14)
- 新装版 リファクタリング―既存のコードを安全に改善する― (OBJECT TECHNOLOGY SERIES)[[rft]] Martin Fowler (著), 児玉 公信 (翻訳), 友野 晶夫 (翻訳), 平澤 章  (翻訳), & 1 その他:
  オーム社; 新装版 (2014/7/26)
- リファクタリング(第2版): 既存のコードを安全に改善する (OBJECT TECHNOLOGY SERIES)[[rft2]] Martin Fowler (著), 児玉 公信 (翻訳), 友野 晶夫 (翻訳), 平澤 章 (翻訳), & 1 その他:
  オーム社; 第2版 (2019/12/1)
- リーダブルコード ―より良いコードを書くためのシンプルで実践的なテクニック (Theory in practice)[[rcd]] Dustin Boswell (著), Trevor Foucher (著), 須藤 功平 (解説), 角 征典  (翻訳):
  オライリージャパン; 初版八刷版 (2012/6/23)
- かんたん Ruby (プログラミングの教科書)[[kruby]] すがわらまさのり  (著)
  技術評論社 (2018/6/21)
