...
３で割り切れる場合の結果を `isFizz` 変数に 5で割り切れる場合の結果 `isBuzz` 変数に代入して使えるようにしました。FizzBuzzを返す判定部分にもこの説明用変数を使って壊れていないか確認しましょう。
...
結果を配列にして返したいのですが `each` メソッドでうまくできませんね。ここは `map` メソッドをつつかましょう。
